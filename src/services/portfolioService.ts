...
export class PortfolioService {
  ...
  private static transformAmplifyProject(amplifyProject: any): Project {
    return {
      id: amplifyProject.id || 'unknown',
      title: amplifyProject.title || 'Untitled Project',
      tech: Array.isArray(amplifyProject.tech) ? amplifyProject.tech : [],
      description: amplifyProject.description || 'No description available',
  ...
  static async getProjectsByCategory(category: string): Promise<Project[]> {
    ...
    try {
      const { data: projects, errors } = await client.models.Project.list({
        filter: { category: { eq: category as any } }
      });
